...
import 'package:flutter_job_portal/tablet/tabs/tabs_view.dart';
import 'package:provider/provider.dart';
import 'package:flutter_job_portal/utils/colors.dart';
import 'package:flutter_job_portal/utils/custom_text.dart';

class CustomAnimDesktopTab extends StatelessWidget {
  final List<String> tabs;

  const CustomAnimDesktopTab({super.key, required this.tabs});
      ...
      children: [
          ...
          child: Stack(
            children: [
              ...
              AnimatedPositioned(
                ...
                left: currentIndex * 500 / tabs.length,
                child: Container(
                  height: 42,
                  width: 500 / tabs.length,
                  decoration: BoxDecoration(
                    color: AppColors().tabsColor,
                    borderRadius:
                        _getTabIndicatorBorderRadius(currentIndex, tabs.length),
                  ),
                ),
              ),
        ...
        IndexedStack(
          ...
          children: List.generate(
            tabs.length,
            (index) => TabletStepsView(steps: _getStepsData()[index]),
          ),
        ),
      ],
    );
